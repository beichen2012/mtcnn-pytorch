...
from model.utility import  IOU, IOM
...
root_dir = r'/home/wyj/dataset/WIDER_FACE'
    ...
    while True:
        ...
        for box in gt_bbox:
            ...
            for i in range(pos_samples):
                ...
                #
                # ## 转换为相对量
                # ox = ox / OUT_IMAGE_SIZE
                # oy = oy / OUT_IMAGE_SIZE
                #
                # # 这里保存成左上角点和右下角点,但仍用ow, oh表示
                # ow = ox + ow / OUT_IMAGE_SIZE
                # oh = oy + oh / OUT_IMAGE_SIZE
                #ow = np.log(ow / OUT_IMAGE_SIZE)
                #oh = np.log(oh / OUT_IMAGE_SIZE)
